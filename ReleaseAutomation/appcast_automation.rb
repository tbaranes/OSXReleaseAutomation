...
class AppCast
  ...
  YAML_FOLDER_PATH = "#{ENV['PROJECT_DIR']}/OSX/ReleaseAutomation"
  FILES_OUTPUT = "#{YAML_FOLDER_PATH}/build"
  ...
  def appcast_setup
    ...
    @appcast_proj_folder   = "#{FILES_OUTPUT}/#{@proj_name}_#{@version}".chomp
  ...
  def remove_old_zip_create_new_zip
	...
	`ditto -x -k "#{@archive_filename}" "#{@appcast_proj_folder}"`
  end
